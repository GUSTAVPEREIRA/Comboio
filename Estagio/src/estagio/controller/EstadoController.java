...
public class EstadoController implements Initializable {
    ...
    private void OnActionGravar(ActionEvent event) {
        ...
        {
            estado.setEst_nome(txt_nome.getText().toString());
        }
        ...
        {
            estado.setEst_uf(txt_sigla.getText().toString());
        }
        ...
        {
            estado.setEst_id(Integer.parseInt(txt_codigo.getText()));
        }
        
        if (erro != true) 
        {
            if (estado.getEst_id()== 0) {
    ...
    {
        this.estado = estado;
        txt_nome.setText(estado.getEst_nome());
        txt_codigo.setText(""+estado.getEst_id());
        txt_sigla.setText(estado.getEst_uf());        
        ativaTela();
    }
    ...
    private void OnActionExcluir(ActionEvent event) {
        ...
        dialogoExe.setTitle("");
        dialogoExe.setHeaderText("Você deseja realmente excluir "+estado.getEst_nome()+"?");
            ...
            if (b == btnSim)
            {
                if (txt_codigo.getText() != "0" && !txt_codigo.getText().isEmpty()) 
                {
                  ...
                  if (deletado == true) 
                  {
                    ...
                    alert.setContentText("Estado "+estado.getEst_nome()+" Excluido");
                    alert.show();             
                    desativaTela();
                  }
                  else
                  {
                    alert.setContentText("Estado "+estado.getEst_nome()+" não pode ser excluido, verifique se há cidades com o estado.");
                    alert.show();       
                  }
